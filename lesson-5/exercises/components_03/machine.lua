...
Machine.OPCODES.BZ        = 0xA2 -- Pop 1 element TOS(0) and jump (branch) relative pc <- (memory[pc] & 0xffffffff - 0x7fffffff) if TOS(0) is 0
Machine.OPCODES.BNZ       = 0xA3 -- Pop 1 element TOS(0) and jump (branch) relative pc <- (memory[pc] & 0xffffffff - 0x7fffffff) if TOS(0) is not 0
...
function Machine:step ()
    ...
    if self.trace_ then
        utils.printStep(pc, op_variant, operand, self:tos())
    end
end
