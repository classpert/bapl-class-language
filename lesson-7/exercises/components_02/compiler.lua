...
local grammar = lpeg.P{
    ...
    lhs         = lpeg.Ct(variable * (T"[" * expression * T"]")^0) / processIndex
                + variable,
    params      = (identifier^1 + T"("*T")") / node("params"), 
    fun         = (R"fun" * identifier * params * block) / node("fun", "identifiers", "params", "block")
                  + (R"rec" * R"fun" * identifier * params * block
                  * (R"and" * identifier * params * block)^0) / node("fun", "identifier", "params", "block"),
...
function Compiler:new ()
    local compiler = {env_ = Tree:new({parent = nil, vars = {}}), code_ = {}, break_ctx_ = Stack:new()}
...
function Compiler:codeGenBlock(ast)
    self.env_ = Tree:new({parent = self.env_, vars = {}})
...
function Compiler:genProgramImage()
    ...
    self:genData(self.env_, self.code_)
